        ...
        <div className="relative cursor-pointer" onClick={openModal}>
          ...
          {hasMultipleImages && (
            <>
              ...
              <button
                onClick={prevImage}
                className="absolute left-2 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10"
              ...
              <button
                onClick={nextImage}
                className="absolute right-2 top-1/2 -translate-y-1/2 bg-black/60 hover:bg-black/80 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10"
          ...
          <div className="absolute inset-0 bg-black bg-opacity-40 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-500">
            <button className="bg-[#D08A64] text-white py-2 px-4 rounded-full flex items-center space-x-2 font-semibold transform scale-90 group-hover:scale-100 transition-transform duration-300">
              <EyeIcon />
          ...
          >
            ...
            {hasMultipleImages && (
              <>
                ...
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 bg-black/60 rounded-lg p-4 z-10">
                    ...
                    {allImages.map((img, index) => (
                      <button
                        ...
                        className={`flex-shrink-0 w-16 h-16 rounded overflow-hidden border-2 transition-all ${
                          index === modalImageIndex ? 'border-white scale-110' : 'border-transparent opacity-60 hover:opacity-100'
                        }`}
                      >
                        <img 
                          src={img} 
            ...
            <div className="absolute bottom-4 left-4 bg-black/60 text-white p-4 rounded-lg max-w-md z-10">
              <p className="text-sm font-semibold text-[#EAD7C4] mb-1">{project.category}</p>
