...
const FileCard: React.FC<{ file: UploadedFile; onDelete: () => void }> = ({ file, onDelete }) => {
    return (
...
const AdminPanel: React.FC<AdminPanelProps> = ({ projects, setProjects }) => {

    const handleFileUpload = (files: FileList | null, type: 'image' | 'video', projectId: number) => {
        if (!files) return;
        ...
        Array.from(files).forEach(file => {
            ...
            reader.onload = (e) => {
                ...
                setProjects(prevProjects => prevProjects.map(p => {
                    if (p.id === projectId) {
                        const newMedia = p.media ? [...p.media, newFile] : [newFile];
    ...
    const handleDelete = (projectId: number, fileId: string) => {
                        ...
                        {(project.media && project.media.length > 0) ? (
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                                {project.media.map(file => (
                                    <FileCard key={file.id} file={file} onDelete={() => handleDelete(project.id, file.id)} />
                                ))}
                            </div>
                        ) : (
